...
.run(function(Offline){Offline.Followers=this;console.log("setf");console.log(Offline.Followers)})
...
.service('Followers', function($rootScope,$q, $state, Funds, Auth, Follower, Users, Loading, Offline) {
    ...
    this.getAlluser = function() {
         ...
         var promise = firebase.database().ref('/users/')
              .once('value').then(function(snapshot) {
                 var usercustom = snapshot.val();
                    usercustom.uid = snapshot.key;
          return $q.resolve(usercustom);

        });
               return Loading.progress(promise);
    }

    this.save=function(aInivite)
    {
        ...
        if($rootScope.online)
        {
            ...
            var promise = ref.set({
                uid: aInivite.uid,
                email: aInivite.email,
                active: aInivite.active,
                activities: aInivite.activities,
                date: aInivite.date.getTime(),
            });
    ...
    this.delete=function(aInvite)
    {
        ...
        {
            Offline.deleteInvite(aInvite);
        }

    }

    this.deleteByID=function(aId)
    {
        if($rootScope.online)
        {
            var ref = firebase.database().ref('/followers').child(aId);
            ref.remove();
...
.factory('Follower', function(Loading) {
    ...
    function Follower(info) {
        ...
        this.active = (info && info.active) || false;
